...
#### 特徴
- **入力**: 日次データ（Confirmed=確定+A、B、C、D、月、日）
- **出力**: 当月末のMP予測値
- **手法**: Random Forest + 月別転換率のハイブリッド（比率を交差検証で自動最適化）
...
#### 主な機能
- **Section 4.5**: Leave-One-Month-Out CVでRF比率を自動選択（交差検証MAEを表示）
- 日ごとの予測精度推移の可視化
- 月別・日別の転換率分析
- 月内予測の進捗シミュレーション（最新月 / 任意の過去月）
- Confirmed（確定+A）と追加予測分の内訳表示
...
#### 特徴
- **入力**: 日次の次月データ（次月確定+A=KA、次月B、次月C、次月D、月、日）
- **出力**: 当月末時点の 次月確定+次月A（KA）の予測値・成長量
- **手法**: Random Forest（特徴量：KA、Next_B、Next_C、Next_D、日付、月）
...
### モデル1: 当月末MP予測
- **精度指標**: Section 4.5 実行後に表示される **交差検証MAE**（データに依存）
- **注意**: Section 5 の In-sample R²/MAE は学習データへの適合度であり、実際の精度ではありません
- **訓練データ**: 完全月（最終データ日 = カレンダー月末日）のみ使用

### モデル2: 次月スタートMP予測
- **精度指標**: Section 5 で表示される MAE・R²（純粋な RF、In-sample）
- **訓練データ**: 完全月のみ使用
